...
void random(unsigned char* publicKey, unsigned char* nonce, unsigned char* output, unsigned int outputSize)
{
    ...
    *((__m256i*) & state[32]) = *((__m256i*)nonce);
    ZeroMemory(&state[64], sizeof(state) - 64);

    for (unsigned int i = 0; i < outputSize / sizeof(state); i++)
    {
        KeccakP1600_Permute_12rounds(state);
        CopyMemory(output, state, sizeof(state));
        output += sizeof(state);
    }
    if (outputSize % sizeof(state))
    {
        KeccakP1600_Permute_12rounds(state);
        CopyMemory(output, state, outputSize % sizeof(state));
    }
}
...
{ // Computes wNAF recoding of a scalar, where digits are in set {0,+-1,+-3,...,+-(2^(w-1)-1)}
    ...
    ZeroMemory(&digits[index], 65 - index);
}
...
static void getIdentity(unsigned char* publicKey, char* identity, bool isLowerCase)
{
    for (int i = 0; i < 4; i++)
    {
...
struct ContractIPOBidTransaction
{
    unsigned char sourcePublicKey[32];
    unsigned char destinationPublicKey[32];
    long long amount;
...
static HWND hWnd, hSeed, hId, hEnergy, hStatus, hInfo1, hInfo2, hInfo3, hTab;
static HWND hTab0TransfersLabel, hTab0Transfers, hTab0DestinationLabel, hTab0Destination, hTab0AmountLabel, hTab0Amount, hTab0Transfer;
static HWND hTab1BidsLabel, hTab1Bids, hTab1QuantityLabel, hTab1Quantity, hTab1PriceLabel, hTab1Price, hTab1AmountLabel, hTab1Amount, hTab1Bid;
static unsigned char arbitratorPublicKey[32];
static char originalNode[1024];
static unsigned char node[4] = { 0, 0, 0, 0 };
static volatile bool requestingThreadIsBusy = false;
static char requestResponseBuffer[sizeof(RequestResponseHeader) + 65535];
static Computors computors;
static short epoch = -1;
static unsigned int prevTick0 = 0, prevTick1 = 0, tick = 0;
static unsigned char timerPhase = 0;
static unsigned char ownSubseed[32], ownPublicKey[32];
static unsigned int pendingTransactionTick = 0;

static void displayMessage(DWORD messageId)
{
    char* message;
    FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, messageId, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&message, 0, NULL);
    SetWindowText(hStatus, message);
    LocalFree(message);

    Sleep(STATUS_DISPLAY_DURATION);
}

static void prettifyNumber(unsigned long long number, char* numberText)
{
    char text[32];
    char textLength = 0;
    do
    ...
    while (--textLength >= 0)
    {
        numberText[dstIndex++] = text[textLength];
        if (!(textLength % 3) && textLength)
        {
            numberText[dstIndex++] = '\'';
        }
    }
    numberText[dstIndex] = 0;
}

static DWORD WINAPI requestingThreadProc(LPVOID lpParameter)
{
    char statusMessage[1024];

    SetWindowText(hStatus, "Creating a socket...");
    SOCKET ownSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
    if (ownSocket == INVALID_SOCKET)
    {
        displayMessage(WSAGetLastError());
    }
    else
    {
        lstrcpy(statusMessage, "Connecting to ");
        lstrcat(statusMessage, originalNode);
        lstrcat(statusMessage, "...");
        SetWindowText(hStatus, statusMessage);
        sockaddr_in addr;
        ZeroMemory(&addr, sizeof(addr));
        addr.sin_family = AF_INET;
        addr.sin_port = htons(PORT);
        addr.sin_addr.S_un.S_un_b.s_b1 = node[0];
        addr.sin_addr.S_un.S_un_b.s_b2 = node[1];
        addr.sin_addr.S_un.S_un_b.s_b3 = node[2];
        addr.sin_addr.S_un.S_un_b.s_b4 = node[3];
        if (connect(ownSocket, (const sockaddr*)&addr, sizeof(addr)))
        {
            displayMessage(WSAGetLastError());
        }
        else
        {
            lstrcpy(statusMessage, "Sending a request to ");
            lstrcat(statusMessage, originalNode);
            lstrcat(statusMessage, "...");
            SetWindowText(hStatus, statusMessage);
            char* buffer = (char*)&requestResponseBuffer[0];
            unsigned int size = ((RequestResponseHeader*)&requestResponseBuffer[0])->size();
            while (size)
            {
                int numberOfBytes;
                if ((numberOfBytes = send(ownSocket, buffer, size, 0)) <= 0)
                {
                    displayMessage(WSAGetLastError());
            ...
            if (!size && lpParameter)
            {
                ...
                lstrcpy(statusMessage, "Receiving a response from ");
                lstrcat(statusMessage, originalNode);
                lstrcat(statusMessage, "...");
                SetWindowText(hStatus, statusMessage);
                while (size < sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader) || size < ((RequestResponseHeader*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers)])->size())
                {
                    int numberOfBytes;
                    if ((numberOfBytes = recv(ownSocket, &requestResponseBuffer[size], sizeof(requestResponseBuffer) - size, 0)) <= 0)
                    {
                        if (numberOfBytes < 0)
                        {
                            displayMessage(WSAGetLastError());
                ...
                if (isDisconnected
                    || size < sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader) || size < ((RequestResponseHeader*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers)])->size())
                {
                    SetWindowText(hStatus, "Not all data of the response have been received!");
                    Sleep(STATUS_DISPLAY_DURATION);
                }
                else
                {
                    switch (((RequestResponseHeader*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers)])->type())
                    {
                    case BROADCAST_COMPUTORS:
                    {
                        Computors* response = (Computors*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader)];
                        if (response->epoch > computors.epoch)
                        {
                            unsigned char digest[32];
                            KangarooTwelve((unsigned char*)response, sizeof(Computors) - SIGNATURE_SIZE, digest, sizeof(digest));
                            if (verify(arbitratorPublicKey, digest, response->signature))
                            {
                                CopyMemory(&computors, response, sizeof(Computors));
                    ...
                    {
                        CurrentTickInfo* response = (CurrentTickInfo*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader)];
                        lstrcpy(statusMessage, NAME);
                        lstrcat(statusMessage, " [");
                        char number[16];
                        itoa(response->numberOfAlignedVotes + response->numberOfMisalignedVotes, number, 10);
                        lstrcat(statusMessage, number);
                        lstrcat(statusMessage, ".");
                        itoa(tick = response->tick, number, 10);
                        lstrcat(statusMessage, number);
                        lstrcat(statusMessage, ".");
                        itoa(epoch = response->epoch, number, 10);
                        lstrcat(statusMessage, number);
                        lstrcat(statusMessage, "]");
			...
			shouldSendTransactionTick = true;
                    ...
                    case RESPOND_ENTITY:
                    {
                        RespondedEntity* response = (RespondedEntity*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader)];
                        if (EQUAL(*((__m256i*)response->entity.publicKey), *((__m256i*)ownPublicKey)))
                        {
                            char numberText[32];
                    ...
                    case RESPOND_CONTRACT_IPO:
                    {
                        RespondContractIPO* response = (RespondContractIPO*)&requestResponseBuffer[sizeof(RequestResponseHeader) + sizeof(ExchangePublicPeers) + sizeof(RequestResponseHeader)];
                        switch (response->contractIndex)
                        {
                        case QUOTTERY_CONTRACT_INDEX:
                        {
                            bool wasEmpty = !ListView_GetItemCount(hTab1Bids);
                            LVITEM item;
                            item.mask = LVIF_TEXT;
                            char text[60 + 1];
                            item.pszText = text;
                            for (unsigned int i = 0; i < NUMBER_OF_COMPUTORS; i++)
                            {
                                ...
                                if (wasEmpty)
                                {
                                    ListView_InsertItem(hTab1Bids, &item);
                                }
                                else
                                {
                                    ListView_SetItem(hTab1Bids, &item);
                                }
                                item.iSubItem = 1;
                                if (EQUAL(*((__m256i*)response->publicKeys[i]), *((__m256i*)ownPublicKey)))
                                {
                                    lstrcpy(text, "[My own id]");
                                }
                                else
                                {
                                    getIdentity(response->publicKeys[i], text, false);
                                }
                                ListView_SetItem(hTab1Bids, &item);
                                item.iSubItem = 2;
                                prettifyNumber(response->prices[i], text);
                                ListView_SetItem(hTab1Bids, &item);
                            }
                            lstrcpy(text, "Bids (tick ");
                            char number[16];
                            _itoa(response->tick, number, 10);
                            lstrcat(text, number);
                            lstrcat(text, ")");
                            SetWindowText(hTab1BidsLabel, text);
                        }
                        break;
                        }
                    }
        ...
        lstrcpy(statusMessage, "Disconnecting from ");
        lstrcat(statusMessage, originalNode);
        lstrcat(statusMessage, "...");
        SetWindowText(hStatus, statusMessage);
...
static LRESULT WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        if (lParam == (LPARAM)hSeed)
        {
            ...
            case EN_CHANGE:
            {
                ...
                GetWindowText(hSeed, (LPSTR)seed, sizeof(seed));
                if (getSubseed(seed, ownSubseed))
                {
                    ...
                    Edit_SetReadOnly(hSeed, TRUE);
                    SetWindowText(hId, id);
                    ShowWindow(hInfo1, SW_HIDE);
                    ShowWindow(hInfo2, SW_HIDE);
                    ShowWindow(hInfo3, SW_HIDE);
                    ShowWindow(hTab, SW_SHOW);
                    //ShowWindow(hTab0TransfersLabel, SW_SHOW);
                    //ShowWindow(hTab0Transfers, SW_SHOW);
                    ShowWindow(hTab0DestinationLabel, SW_SHOW);
                    ShowWindow(hTab0Destination, SW_SHOW);
                    ShowWindow(hTab0AmountLabel, SW_SHOW);
                    ShowWindow(hTab0Amount, SW_SHOW);
                    ShowWindow(hTab0Transfer, SW_SHOW);
        ...
        else if ((lParam == (LPARAM)hTab0Destination) || (lParam == (LPARAM)hTab0Amount))
        {
            switch (HIWORD(wParam))
            {
            case EN_CHANGE:
            {
                ...
                {
                    EnableWindow(hTab0Transfer, FALSE);
                }
                else
                {
                    EnableWindow(hTab0Transfer, TRUE);
                }
            }
            break;
            }
        }
        else if ((lParam == (LPARAM)hTab1Quantity) || (lParam == (LPARAM)hTab1Price))
        {
            switch (HIWORD(wParam))
            {
            case EN_CHANGE:
            {
                char quantityText[16], priceText[16];
                GetWindowText(hTab1Quantity, quantityText, sizeof(quantityText));
                GetWindowText(hTab1Price, priceText, sizeof(priceText));
                const long long quantity = _atoi64(quantityText);
                const long long price = _atoi64(priceText);
                if (quantity <= 0 || quantity > NUMBER_OF_COMPUTORS
                    || price <= 0 || price > MAX_AMOUNT / NUMBER_OF_COMPUTORS)
                {
                    SetWindowText(hTab1Amount, "");
                    EnableWindow(hTab1Bid, FALSE);
                }
                else
                {
                    char numberText[32];
                    prettifyNumber(quantity * price, numberText);
                    SetWindowText(hTab1Amount, numberText);
                    EnableWindow(hTab1Bid, TRUE);
                }
            }
            break;
            }
        }
        else if (lParam == (LPARAM)hTab0Transfer)
        {
            switch (HIWORD(wParam))
            {
            case BN_CLICKED:
            {
                if (tick < pendingTransactionTick)
                {
                    char text[256];
                    lstrcpy(text, "There is a pending transaction at tick ");
                    char number[16];
        ...
        else if (lParam == (LPARAM)hTab1Bid)
        {
            switch (HIWORD(wParam))
            {
            case BN_CLICKED:
            {
                ...
                else
                {
                    ...
                    if (quantity > 0 && quantity <= NUMBER_OF_COMPUTORS
                        && price > 0 && price <= MAX_AMOUNT / NUMBER_OF_COMPUTORS)
                    {
                        ContractIPOBidTransaction request;
                        *((__m256i*)request.sourcePublicKey) = *((__m256i*)ownPublicKey);
                        *((__m256i*)request.destinationPublicKey) = ZERO;
                        *((unsigned int*)request.destinationPublicKey) = QUOTTERY_CONTRACT_INDEX;
                        request.amount = 0;
                        pendingTransactionTick = request.tick = tick + TICK_OFFSET;
                        request.inputType = CONTRACT_IPO_BID;
                        request.inputSize = sizeof(request.price) + sizeof(request.quantity) + sizeof(request.padding);
                        request.price = price;
                        request.quantity = quantity;
                        unsigned char digest[32];
                        KangarooTwelve((unsigned char*)&request, offsetof(ContractIPOBidTransaction, signature), digest, sizeof(digest));
                        ...
                        if (!initiateRequest(BROADCAST_TRANSACTION, &request, sizeof(request), false))
                        {
                            ...
                            MessageBox(hWnd, "Impossible to bid right now!\n\nTry again in a few moments, please.", "Warning", MB_ICONWARNING);
                        }
                        else
                        {
                            char text[256];
                            lstrcpy(text, "The bid has been published and is supposed to be recorded into the blockchain at tick ");
                            char number[16];
                            _itoa(pendingTransactionTick, number, 10);
                            lstrcat(text, number);
                            lstrcat(text, ".\n\nYou may repeat the bid, if it is not recorded.");
    ...
    case WM_NOTIFY:
    {
        ...
        if (hdr->hwndFrom == hTab)
        {
            switch (hdr->code)
            {
            case TCN_SELCHANGE:
            {
                ShowWindow(hTab0TransfersLabel, SW_HIDE);
                ShowWindow(hTab0Transfers, SW_HIDE);
                ShowWindow(hTab0DestinationLabel, SW_HIDE);
                ShowWindow(hTab0Destination, SW_HIDE);
                ShowWindow(hTab0AmountLabel, SW_HIDE);
                ShowWindow(hTab0Amount, SW_HIDE);
                ShowWindow(hTab0Transfer, SW_HIDE);
                ShowWindow(hTab1BidsLabel, SW_HIDE);
                ShowWindow(hTab1Bids, SW_HIDE);
                ShowWindow(hTab1QuantityLabel, SW_HIDE);
                ShowWindow(hTab1Quantity, SW_HIDE);
                ShowWindow(hTab1PriceLabel, SW_HIDE);
                ShowWindow(hTab1Price, SW_HIDE);
                ShowWindow(hTab1AmountLabel, SW_HIDE);
                ...
                case 1:
                {
                    ShowWindow(hTab1BidsLabel, SW_SHOW);
                    ShowWindow(hTab1Bids, SW_SHOW);
                    ShowWindow(hTab1QuantityLabel, SW_SHOW);
                    ShowWindow(hTab1Quantity, SW_SHOW);
                    ShowWindow(hTab1PriceLabel, SW_SHOW);
                    ShowWindow(hTab1Price, SW_SHOW);
                    ShowWindow(hTab1AmountLabel, SW_SHOW);
                    ShowWindow(hTab1Amount, SW_SHOW);
                    ShowWindow(hTab1Bid, SW_SHOW);
                }
                break;
                }
            }
    ...
    case WM_TIMER:
    {
        ...
        case 1:
        {
            if (!timerPhase)
            {
                ...
                {
                    if (epoch != computors.epoch)
                    {
                        initiateRequest(REQUEST_COMPUTORS, NULL, 0, true);
                    }
                    else
                    {
                        initiateRequest(REQUEST_CURRENT_TICK_INFO, NULL, 0, true);
                    }
                }
            }
            else
            {
                ...
                case 1:
                {
                    if (tick != prevTick1)
                    {
                        ...
                        RequestContractIPO request;
                        request.contractIndex = QUOTTERY_CONTRACT_INDEX;
                        initiateRequest(REQUEST_CONTRACT_IPO, &request, sizeof(request), true);
                    }
                    else
                    {
                        ////
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow)
{
    if (__argc < 2)
    {
        MessageBox(NULL, "Provide the IP address of a Qubic node as the first command-line argument!", NAME, MB_ICONEXCLAMATION);
    }
    else
    {
        getPublicKeyFromIdentity((unsigned char*)ARBITRATOR, arbitratorPublicKey);
        computors.epoch = 0;
      ...
      unsigned char seed[55 + 1];
        ...
        CreateWindow(WC_STATIC, "Seed", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 10, 10, 40, 20, hWnd, NULL, hInstance, NULL);
        hSeed = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | ES_PASSWORD, 50, 10, 190, 20, hWnd, NULL, hInstance, NULL);
        Edit_LimitText(hSeed, 55);
        CreateWindow(WC_STATIC, "Id", WS_CHILD | WS_VISIBLE | SS_CENTERIMAGE, 260, 10, 20, 20, hWnd, NULL, hInstance, NULL);
        hId = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | WS_VISIBLE | ES_AUTOHSCROLL | ES_READONLY, 280, 10, 210, 20, hWnd, NULL, hInstance, NULL);
        ...
        hTab = CreateWindow(WC_TABCONTROL, NULL, WS_CHILD, rect.left, rect.top + 40, rect.right - rect.left, rect.bottom - rect.top - (statusRect.bottom - statusRect.top) - 40, hWnd, NULL, hInstance, NULL);
        {
            TCITEM item;
            item.mask = TCIF_TEXT;
            item.pszText = (LPSTR)"$";
            TabCtrl_InsertItem(hTab, 0, &item);
            item.pszText = (LPSTR)"Quottery (IPO)";
        ...
        hTab0TransfersLabel = CreateWindow(WC_STATIC, "Transfers", WS_CHILD | SS_CENTERIMAGE, tabRect.left + 10, rect.top + 40 + tabRect.top + 10, (tabRect.right - tabRect.left) - 230 - tabRect.left, 20, hWnd, NULL, hInstance, NULL);
        hTab0Transfers = CreateWindow(WC_LISTVIEW, NULL, WS_BORDER | WS_CHILD | LVS_NOSORTHEADER | LVS_REPORT | LVS_SHOWSELALWAYS, tabRect.left + 10, rect.top + 40 + tabRect.top + 30, (tabRect.right - tabRect.left) - 230 - tabRect.left, (tabRect.bottom - tabRect.top) - 40, hWnd, NULL, hInstance, NULL);
        ListView_SetExtendedListViewStyle(hTab0Transfers, LVS_EX_FLATSB | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
        hTab0DestinationLabel = CreateWindow(WC_STATIC, "Destination", WS_CHILD | SS_CENTERIMAGE, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 10, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab0Destination = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | ES_AUTOHSCROLL, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 30, 200, 20, hWnd, NULL, hInstance, NULL);
        Edit_LimitText(hTab0Destination, 60);
        hTab0AmountLabel = CreateWindow(WC_STATIC, "Amount (qus)", WS_CHILD | SS_CENTERIMAGE, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 60, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab0Amount = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | ES_NUMBER | ES_RIGHT, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 80, 200, 20, hWnd, NULL, hInstance, NULL);
        Edit_LimitText(hTab0Amount, 15);
        hTab0Transfer = CreateWindow(WC_BUTTON, "Transfer", WS_CHILD | WS_DISABLED | BS_CENTER | BS_FLAT | BS_VCENTER, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 110, 200, 60, hWnd, NULL, hInstance, NULL);

        hTab1BidsLabel = CreateWindow(WC_STATIC, "Bids", WS_CHILD | SS_CENTERIMAGE, tabRect.left + 10, rect.top + 40 + tabRect.top + 10, (tabRect.right - tabRect.left) - 230 - tabRect.left, 20, hWnd, NULL, hInstance, NULL);
        hTab1Bids = CreateWindow(WC_LISTVIEW, NULL, WS_BORDER | WS_CHILD | LVS_NOSORTHEADER | LVS_REPORT | LVS_SHOWSELALWAYS, tabRect.left + 10, rect.top + 40 + tabRect.top + 30, (tabRect.right - tabRect.left) - 230 - tabRect.left, (tabRect.bottom - tabRect.top) - 40, hWnd, NULL, hInstance, NULL);
        ListView_SetExtendedListViewStyle(hTab1Bids, LVS_EX_FLATSB | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
        {
            LVCOLUMN column;
            ...
            column.iSubItem = 0;
            ListView_InsertColumn(hTab1Bids, 0, &column);
            column.cx = (tabRect.right - tabRect.left) - 380 - tabRect.left - GetSystemMetrics(SM_CXBORDER) * 2 - GetSystemMetrics(SM_CXVSCROLL);
            column.pszText = (LPSTR)"Id";
            column.iSubItem = 1;
            ListView_InsertColumn(hTab1Bids, 1, &column);
            column.fmt = LVCFMT_RIGHT;
            column.cx = 120;
            column.pszText = (LPSTR)"Price (qus)";
            column.iSubItem = 2;
            ListView_InsertColumn(hTab1Bids, 2, &column);
        }
        hTab1QuantityLabel = CreateWindow(WC_STATIC, "Quantity (from 1 to 676)", WS_CHILD | SS_CENTERIMAGE, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 10, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab1Quantity = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | ES_NUMBER | ES_RIGHT, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 30, 200, 20, hWnd, NULL, hInstance, NULL);
        Edit_LimitText(hTab1Quantity, 15);
        hTab1PriceLabel = CreateWindow(WC_STATIC, "Price (qus)", WS_CHILD | SS_CENTERIMAGE, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 60, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab1Price = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | ES_NUMBER | ES_RIGHT, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 80, 200, 20, hWnd, NULL, hInstance, NULL);
        Edit_LimitText(hTab1Price, 15);
        hTab1AmountLabel = CreateWindow(WC_STATIC, "Amount (qus)", WS_CHILD | SS_CENTERIMAGE, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 110, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab1Amount = CreateWindow(WC_EDIT, NULL, WS_BORDER | WS_CHILD | ES_READONLY | ES_RIGHT, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 130, 200, 20, hWnd, NULL, hInstance, NULL);
        hTab1Bid = CreateWindow(WC_BUTTON, "Bid", WS_CHILD | WS_DISABLED | BS_CENTER | BS_FLAT | BS_VCENTER, (tabRect.right - tabRect.left) - 210, rect.top + 40 + tabRect.top + 160, 200, 60, hWnd, NULL, hInstance, NULL);

        ShowWindow(hWnd, nCmdShow);
        SetFocus(hSeed);

        lstrcpy(originalNode, __argv[1]);
        sscanf(originalNode, "%d.%d.%d.%d", &node[0], &node[1], &node[2], &node[3]);
        WSADATA wsaData;
